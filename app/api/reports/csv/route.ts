...
export async function GET() {
  ...
  const rows = ["Date,Customer,Quantity,Price,Amount,Status"];
  deliveries.forEach((d) => {
    const amount = d.quantity * d.pricePerCylinder;
    rows.push(`${new Date(d.deliveryDate).toISOString().slice(0,10)},${d.customerId?.name || ""},${d.quantity},${d.pricePerCylinder},${amount},${d.status}`);
  });
  const csv = rows.join("\n");
